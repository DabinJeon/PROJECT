...
def brush(event):
    ...
    try:
        if (event.x > 0 or event.x < 512) and (event.y > 0 or event.y < 512):
...
def erase():
    ...
    global inImageR, inImageG, inImageB, outImageR, outImageG
    global outImageB, filename, cvPhoto, prediction
...
def prediction_image():
    ...
    model = load_model('skm_classifier10.h5')
...
def openOpenCV():
    global window, inW, inH, outW, outH, inImageR, inImageG, inImageB
    global outImageR, outImageG, outImageB, filename, photo, cvPhoto, prediction, imageData

    if prediction == "사과":
        filename = 'm_image/apple.jpg'
    elif prediction == "버스":
        filename = 'm_image/bus.jpg'
    elif prediction == "구름":
        filename = 'm_image/cloud.jpg'
    elif prediction == "안경":
        filename = 'm_image/glasses.jpg'
    elif prediction == "모니터":
        filename = 'm_image/monitor.jpg'
    elif prediction == "바지":
        filename = 'm_image/pants.jpg'
    elif prediction == "연필":
        filename = 'm_image/pencil.jpg'
    elif prediction == "토끼":
        filename = 'm_image/rabbit.jpg'
    elif prediction == "해":
        filename = 'm_image/sun.jpg'
    elif prediction == "태극기":
...
def loadImageColorCV2(imageData):
    ...
    cvPhoto = cv2.cvtColor(imageData, cv2.COLOR_BGR2RGB)
...
def displayImageColor():
    global window, canvas, paper, inW, inH, outW, outH, inImageR, inImageG, inImageB
    global outImageR, outImageG, outImageB, filename, canvas2, prediction
...
def search():
    global prediction
    url = "https://www.google.co.in/search?q=" + prediction + "&source=lnms&tbm=isch"
    # chromedriver 경로 (다운로드 필요)
